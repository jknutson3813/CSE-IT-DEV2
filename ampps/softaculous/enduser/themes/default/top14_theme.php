...
<![endif]-->
<link rel="stylesheet" type="text/css" href="'.$fullurl.'/themes/default/css/font-awesome-5.css?'.$GLOBALS['globals']['version'].'">
<style type="text/css">
...
if(!empty($updates_available)){	
	...
	if(!empty($my_ins) && !empty($user['ins'])){
		...
		foreach($user_ins as $uk => $uv){
			...
			// custom scripts logos path
			if($uv['sid'] > 10000){
				if((is_cp_theme('paper_lantern') || is_cp_theme('jupiter')) && file_exists($theme['images'].'topscripts/48/'.$uv['sid'].'__logo.gif')){
			...
			$str .= '<td width="'.$td_width.'" valign="middle" align="center" height="'.$td_height.'"><a href="'.$fullurl.'/'.$globals['ind'].'act=editdetail&insid='.$uv['insid'].'" style="text-decoration:none;'.$softac_cat_color.'"><img src="'.($uv['sid'] > 10000 ? $cscript_img_path : $url.$img_path.'"').' border="0" id="top14scriptbg"/></a><br />'.(!empty($has_sign_on) ? '<a href="'.$fullurl.'/'.$globals['ind'].'act=sign_on&insid='.$uv['insid'].'&autoid='.srandstr(32).'" target="_blank" style="text-decoration:none;">'.(is_cp_theme('paper_lantern') || is_cp_theme('jupiter') || is_cp_theme('x3') ? '<i class="fas fa-user fa-1x" style="font-size:1.1em;"></i>' : '<img src="'.$fullurl.'/'.$cat_image_path.'php_blogs.gif" title="" style="height:13;width:13;vertical-align:bottom;" border="0">').'</a>&nbsp;' : '').'<a href="'.$fullurl.'/'.$globals['ind'].'act=editdetail&insid='.$uv['insid'].'" style="text-decoration:none; vertical-align:top;'.$softac_cat_color.'">'.(!empty($uv['site_name']) ? (!empty($_site_name) ? stripslashes($_site_name) : stripslashes($uv['site_name'])) : $iscripts[$uv['sid']]['name']).'</a></td>';
		}
...
$str .= '<div class="softac_cat" style="vertical-align:middle;padding:5px;text-align:left;background-color: '.$softac_cat_bgcolor.';width:'.$width.';'.$softac_cat_color.'"><b>'.$l['category'].' :</b></div>
		...
		foreach($allcatwise as $type => $cats){
			foreach($cats as $k => $v){
				...
				$str .= ' <td width="'.$td_width.'" valign="top" align="center"><a href="'.$fullurl.'/'.$globals['index'].'ind='.$type.'&act=listsoftwares&cat='.$k.'" style="text-decoration:none;'.$softac_cat_color.'" class="desc">'.((is_cp_theme('paper_lantern') || is_cp_theme('jupiter') || is_cp_theme('x3')) ? '<div class="top14catbg"><i class="fas sai-'.$k.(is_cp_theme('x3') ? ' fa-1x' : ' fa-2x').'"></i></div>' : '<img src="'.$fullurl.'/'.$cat_image_path.$type.'_'.$k.'.gif" title="'.strtoupper($type).' '.strtoupper($k).'" border="0">').(!is_cp_theme('x3') ? '<br />' : '').'<div  id="top14catmargin">'.$l['cat_'.$type.'_'.$k].'</div></a></td>';
