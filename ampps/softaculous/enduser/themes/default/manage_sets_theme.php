...
echo '<div class="bg" style="min-height:400px;"><br />
		<center class="sai_main_head" style="width:100%;" align="center">
			<i class="fas fa-chart-pie fa-2x" style="color:#00A0D2; vertical-align: middle;"></i> &nbsp;'.$l['<title>'].'
		</center><hr>';
...
<input name="create_sets" class="sai-button" id="create_sets" value="'.$l['create_sets'].'" type="button">&nbsp;&nbsp;
<input name="remove_sets" class="sai-button" id="remove_sets" value="'.$l['remove_sets'].'" type="button">
<a role="button" href="'.$globals['ind'].'act=wordpress" style="float:right;"><span class="btn btn-outline-primary">'.$l['return_to_wpm'].'</span></a>
<br/><br/>
	<form accept-charset="'.$globals['charset'].'" name="manage_plugin_theme" method="post" action="">
		<div class="row mb-4" id="add_sets_div" style="display:none;">
			<div class="col-12">
				<input type="text" name="set_input" class="form-control w-50 d-inline-block mb-2 mb-md-0" id="set_input" size="30" placeholder="'.$l['name_of_set'].'"/>
				<input name="add_sets" class="sai-button d-inline-block" id="add_sets" value="'.$l['create_set_button'].'" type="submit">
			</div>
		</div>
		'.csrf_display().'
	</form>
	...
	<table width="80%" id="manage_plugin_theme" class="table table-hover text-center" style="margin:0px auto;">
		...
		<tr>
			...
			<th style="cursor:pointer">
				<span class="someclass" title="'.$l['default_exp'].'" style="cursor:pointer;">
					<font class="bboxtxt">'.$l['default_value'].'</font>
				</span>
			</th>
		</tr>
	</thead>';
	...
	foreach($sets as $sk => $sv){
		...
		echo '
		<tr>
			<td style="vertical-align:middle;"><input id="'.$sk.'" type="checkbox" name="sets[]" class="check_allsets"></td>
			...
			<td id="'.$sk.'"><input name="'.$sk.'" class="sai-button p-2" id=""'.$sk.'"" value="'.$l['add_plugins'].'" type="button" style="padding: 2px 6px;" data-toggle="modal" data-target="#search_modal" onclick="add_plugins_themes(this.name, this.value)"></td>
			
			<td class="class_valign"><span id="'.$sk.'_theme_count">'.(!empty($sv['themes']) ? count($sv['themes']).$l['total_plugin_theme'] : '').'</span></td>
			
			<td id="'.$sk.'"><input name="'.$sk.'" class="sai-button p-2" id="add_themes" value="'.$l['add_themes'].'" type="button" style="padding: 2px 6px;" data-toggle="modal" data-target="#search_modal" onclick="add_plugins_themes(this.name, this.value)"></td>

			<td class="class_valign"><span class="someclass" title="'.$l['view_items'].'" style="cursor:pointer;" data-toggle="modal" data-target="#list_set_items_modal" onclick="list_set_items(\''.$sk.'\')"><i class="fas fa-2x fa-list" style="color:#676a6c;margin:3px;"></i></span></td>
			
			<td id="'.$sk.'">
				<div class="dropdown">
					<button class="sai-button p-2 dropdown-toggle" type="button" data-toggle="dropdown" style="padding: 2px 6px;">'.$l['action'].'
					<span class="caret"></span></button>&nbsp;<span id="rem_plth_'.$sk.'"></span>
					<ul class="dropdown-menu p-2">
					<li id="'.implode(",", (array) $sets[$sk]['plugins']).'" class="p-1" data-toggle="modal" data-target="#remove_plugin_theme_modal" onclick="remove_wp_plugins_themes(\''.$sk.'\', \'plugins\')"><a style="text-decoration:none; cursor:pointer;">'.$l['remove_plugins'].'</a></li>
					<li id="'.implode(",", (array) $sets[$sk]['themes']).'" class="p-1" data-toggle="modal" data-target="#remove_plugin_theme_modal" onclick="remove_wp_plugins_themes(\''.$sk.'\', \'themes\')"><a style="text-decoration:none; cursor:pointer;">'.$l['remove_themes'].'</a></li>
					</ul>
				</div>
			</td>
			<td><input class="sai-button p-2" id="softinstall_set_button" value="'.$l['install_set'].'" type="button" style="padding: 2px 6px;" data-toggle="modal" data-target="#softinstall_set_modal" onclick="show_softinstall_set_modal(\''.$sk.'\')"></td>
		...
		<div class="modal-dialog">
		  <div class="modal-content">
			...
			<div class="modal-body">
				<input type="hidden" name="plugin_or_theme" id="plugin_or_theme" value="">
				<input type="hidden" name="plugin_theme_set" id="plugin_theme_set" value="">
				<table id="remove_plugins_themes" class="table table-hover">		
				</table>
			</div>
			<div class="modal-footer">
				'.csrf_display().'
			  <button type="button" class="btn btn-default sai-button" data-dismiss="modal" id="remove_plugin_submit" onclick="remove_plugin_submit()">'.$l['remove'].'</button>
			</div>
		  </div>
		</div>
	</div>
	...
	<div class="modal fade" id="list_set_items_modal" role="dialog">
		<div class="modal-dialog">
		  <div class="modal-content">
			...
			<div class="modal-footer">
			  <button type="button" class="btn btn-default sai-button" data-dismiss="modal">'.$l['ok'].'</button>
			</div>
		  </div>
		</div>
	</div>	
		  ...
		  <div class="modal-content">
			...
			<div class="modal-footer">
				<div id="add_plth" class="wpc_inline"></div>
					'.csrf_display().'
				<button type="button" class="btn btn-default sai-button" id="submit_plugins_themes" name="submit_plugins_themes" onclick="submit_addition();">Add</button>
			</div>
		  </div>
		</div>
	</div>
		...
		<div class="modal-dialog">
		  <div class="modal-content">
			...
			<div class="modal-footer">
				...
				'.csrf_display().'
				<button type="button" class="btn btn-default sai-button" id="softsubmit_install_set" name="softsubmit_install_set" onclick="softsubmit_install_set();">Install</button>
			</div>
		  </div>
		</div>
	</div>';
