	...
	<form accept-charset="'.$globals['charset'].'" name="addbackup" method="post" action="" class="form-horizontal">
		...
		<div class="row">
			<div class="col-12 text-center sai_main_head">
				...
				<span class="form-title">'.$l['add_backup_loc'].'</span>
			</div>
		</div>';
		...
		<div class="row">
			...
			<div class="col-12 col-md-6 col-xl-6 mb-2">
				...
				<select name="protocol" class="form-control" id="protocol">
					...
					(empty($globals['disable_backup_onedrive']) ? '<option value="onedrive" '.(isset($_REQUEST['protocol']) && $_REQUEST['protocol'] == 'onedrive' ? 'selected="selected"' : '').'>'.$l['onedrive'].'</option>' : '').	
					(empty($globals['disable_backup_aws']) ? '<option value="softaws" '.(isset($_REQUEST['protocol']) && $_REQUEST['protocol'] == 'softaws' ? 'selected="selected"' : '').'>'.$l['aws_s3bucket'].'</option>' : '').'
				</select>
			</div>
		</div>
		
		<div id="gdrive_auth_div">
			<div class="col-12 col-md-12 col-xl-12 mb-2 alert alert-warning">'.$l['gdrive_auth_message'].'</div>
		</div>

		<div id="onedrive_auth_div">
			<div class="col-12 col-md-12 col-xl-12 mb-2 alert alert-warning">'.$l['onedrive_auth_message'].'</div>
		</div>
		...
		<div class="row" id="dropbox_authorize"><br/>
			<div class="col-12 col-md-6 col-xl-6 mb-2">
				...
				<span class="sai_exp2">'.$l['dropbox_authorization_exp'].maybe_show_docs('https://softaculous.com/docs/Add_Backup_Location_Dropbox', '&nbsp;&nbsp;'.$l['view_guide']).'</span>
		...
		<p align="center">
			<br /><br />
			'.csrf_display().'
			<input class="btn sai-button" type="submit" name="addbackuploc" value="'.$l['add_backup_loc'].'" />
			<br /><br />
