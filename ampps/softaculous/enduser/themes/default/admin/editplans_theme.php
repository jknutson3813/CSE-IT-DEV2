...
<div class="container-fluid">
	...
	'.(empty($globals['lictype']) ? '<div class="px-4 my-2 text-center">
			<span class="sai_bboxtxt">'.$l['feature_not_available'].'</span>
		</div>' : '');
...
if(!empty($done)){
	echo '<div class="row my-2 text-center">
			<div class="col-12 col-lg-10 alert alert-success p-3 mx-auto">
				<i class="fas fa-check sai_icons mr-2"></i>'.$l['done'].'
			</div>
		</div>';
}
...
echo '<div class="row">
	<div class="col-12 col-lg-6 mb-4">
		...
		if(method_exists($softpanel, 'userdata_cli')){
			echo '<input type="checkbox" name="disable_autoupgrade" id="disable_autoupgrade" '.POSTchecked('disable_autoupgrade', $unserplan['disable_autoupgrade']).'/>
				<label for="disable_autoupgrade" class="sai-label">'.$l['disable_autoupgrade'] .'</label><br />
		...
		<div class="sai-card p-3">
			...
			<div class="sai_form">
				<div class="form-group">
					...
					<span class="sai-label d-block"><input type="checkbox" id="checkallcpplan_1" onclick="checkallcpplan(this);" class="mr-1"/><label for = "checkallcpplan_1" class = "sai-label">'.$l['select_all'].'</label></span>
					<span class="sai-label d-block"><input type="checkbox" id = "cpplansetting_1" name="cpplansetting" class="mr-1"/><label for = "cpplansetting_1" class = "sai-label">'.$l['overwrite_settings'].'</label></span>
				';
				foreach($_cpplan as $k => $v){
					echo '<div class="script_choose" id="td_'.$k.'">
						...
						<a href="javascript:void(0)" style="text-decoration: none; " onclick="selectchkbox(\''.$k.'\',\'cpplan_\');"><font class="sai-label">'.$v['original_key'].'</font></a>
					</div>';
				}
		echo'</div>
		</div>
	...
	<div class="sai-card p-3">
		...
		<div class="sai_form">
			<div class="form-group">
				...
				<span class="sai-label d-block"><input type="checkbox" id="checkallreseller_1" onclick="checkallreseller(this);" class="mr-1"/><label for = "checkallreseller_1" class = "sai-label">'.$l['select_all'].'</label></span>
				<span class="sai-label d-block"><input type="checkbox" id = "resellersetting_1" name="resellersetting" class="mr-1"/><label for = "resellersetting_1" class = "sai-label">'.$l['overwrite_settings'].'</label></span>';
				...
				foreach($_resellers as $k => $v){
					...
					echo '<div class="script_choose" id="td_'.$k.'">
						...
						<img src="'.$theme['images'].'reseller15.png">
						<a href="javascript:void(0)" class="text-decoration-none" onclick="selectchkbox(\''.$k.'\',\'resellers_\');">
	...
	<div class="sai-card p-3">
		...
		<div class="sai_form">
			<div class="form-group">
				...
				<span class="sai-label d-block"><input type="checkbox" id = "checkalluser_2" onclick="checkalluser(this);" class="mr-1"/><label for = "checkalluser_2" class = "sai-label">'.$l['select_all'].'</label></span>
				<span class="sai-label d-block mb-2"><input type="checkbox" id="usersetting_2" name="usersetting" class="mr-1"/><label for = "usersetting_2" class = "sai-label">'.$l['overwrite_settings'].'</label></span>
			';
			foreach($_users as $k=>$v){			
				echo '<div class="script_choose" id="td_'.$k.'">
					...
					<a href="javascript:void(0)" class="text-decoration-none" onclick="selectchkbox(\''.$k.'\',\'users_\');">
						<span class="sai-label">'.$v['original_key'].'</span>
					</a>
				</div>';	
			}
...
<div class="col-12 col-lg-6 mb-4">
	<div class="sai-card p-3">
		...
		<div class="sai_form">
			<div class="form-group">
				<label class="sai_exp2 d-block mb-2 d-block">'.$l['exp_setselect'].'</label>
				<span class="sai-label d-block">
					<input type="checkbox" id="checkallsets_3" onclick="checkallsets(this);"/ class="mr-1"><label for = "checkallsets_3" class = "sai-label">'.$l['select_all'].'</label>
				</span>';
				...
				foreach($all_sets as $sk=> $av){
					...
					echo '<div class="script_choose" id="td_'.$sk.'">
							<input type="checkbox" name="sets_name[]" id="sets_'.$sk.'" '.POSTchecked_Array('sets_name', $sk, (!empty($unserplan['sets'][$sk]) ? true : false)).' value="'.$sk.'" onclick="changebg(\''.$sk.'_admin\', \'sets_\');"/>
							<a href="javascript:void(0)" class="text-decoration-none" " onclick="selectchkbox(\''.$sk.'\', \'sets_\');">
		...
		<div class="sai_form">
			<div class="form-group">
				...
				<span class="sai-label d-block mb-2">
					<input type="checkbox" id = "checkallsoftware_4" onclick="checkallsoftware(this);" class="mr-1"/><label for = "checkallsoftware_4" class = "sai-label">'.$l['select_all'].'</label>
				</span>';
				//r_print($allcatwise);
					...
					foreach($av as $kk => $vv){
						...
						foreach($vv as $k => $v){
							...
							echo '<div class="col-6 col-lg-3 mb-3 script_choose" id="td_'.$k.'">
									<input type="checkbox" id="scripts_'.$k.'" name="scripts_'.$k.'" '.POSTchecked('scripts_'.$k, !empty($unserplan['scripts'][$k])).'  onclick="changebg(\''.$k.'\',\'scripts_\');" class="d-inine-block mr-1"/>
...
echo'<p align="center">
	...
	'.(empty($globals['lictype']) ? '<div class="px-4 my-2 text-center"><span class="sai_bboxtxt">'.$l['feature_not_available'].'</div>' : '').'<br />
	</div>	
</div>
</form><br />';
