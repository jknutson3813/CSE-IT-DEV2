...
function dbexists(id, alrt, re){
	try{
		...
		if(/[^a-zA-Z0-9'.(!empty($softpanel->allowed_db_special_char) ? implode('\\', $softpanel->allowed_db_special_char) : '').']/.test($_(id).value)){
...
$info['install'] .= '<div class="row">
	...
	if(!empty($www)){
		...
		<div class="col-12 col-md-6 mt-2 mb-2">
			<label for="wwwdir" class="form-label">'.$l['wwwdir'].'</label>
			<span class="sai_exp">'.$l['wwwdir_exp'].'</span>
		...
		foreach($settings as $group => $sets){
			...
			foreach($sets as $sk => $sv){
				...
				<div class="script_fields" '.(!empty($sv['quick_install']) ? 'quick_install="1"' : '').'>
						...
						<label class="sai_toggle-pass" for="toggle_pass_admin" style="margin-top:6px;">
							<span id="show_hide_admin">'.$l['hide'].'</span>
						</label>
...
$info['install'] .= '
	<div class="bg custom_install">
		...
		<i id="advoptions_toggle_plus" class="fas fa-plus-square" style="margin-top:-4px;"></i>&nbsp;&nbsp;'.$l['adv_option'].'
		...
		<nav class="navbar navbar-default p-0">
			...
			<div class="collapse navbar-collapse soft-options p-2 px-3" id="myNavbar" data-toggle="collapse" data-target=".navbar-collapse">
				<ul class="nav navbar-nav">
					<li>
						<a href="javascript:tabs.tab(\'overview\')" id="overview" class="sai_tab2 text-decoration-none">'.$l['overview'].'</a>
					</li>
					<div class="dropdown-divider"></div>
					<li>
						<a href="javascript:tabs.tab(\'features\')" id="features" class="sai_tab2 text-decoration-none">'.$l['features'].'</a>
					</li>
					<div class="dropdown-divider"></div>
					<li>'.(empty($globals['off_screenshot_link']) ? '
						<a href="javascript:tabs.tab(\'screenshots\'); loadScreenshots();" class="sai_tab2 text-decoration-none" id="screenshots">'.$l['screenshots'].'</a>' : '').'
					</li>
					<div class="dropdown-divider"></div>
					<li>'.(empty($globals['off_demo_link']) ? '
						<a href="'.($soft >= 10000 && !empty($info['demo']) ? $info['demo'] : $globals['ind'].'act=demos&soft='.$soft).'" class="sai_tab2 text-decoration-none" id="demo" target="_blank">'.$l['demo'].'</a>' : '').'
					</li>
					<div class="dropdown-divider"></div>
					<li>'.(empty($globals['off_rating_link']) && $soft < 10000 ? '
						<a href="javascript:tabs.tab(\'ratings\')" class="sai_tab2 text-decoration-none" id="ratings">'.$l['ratings'].'</a>' : '').'
					</li>
					<div class="dropdown-divider"></div>
					<li>'.(empty($globals['off_review_link']) && $soft < 10000 ? '
						<a href="javascript:tabs.tab(\'reviews\')" class="sai_tab2 text-decoration-none" id="reviews">'.$l['reviews'].'</a>' : '').'
					</li>
					<div class="dropdown-divider"></div>
					<li>'.((empty($info['import']) || !empty($globals['disable_import'])) ? '' : '
						<a href="'.$globals['ind'].'act=import&soft='.$soft.'" class="sai_tab2 text-decoration-none" id="import">'.$l['import'].'</a>').'
					</li>
				</ul>
			</div>
		</nav>
	...
	if((!empty($installations[$soft]) || array_intersect(array_keys($installations), array_keys($software['branch']))) && empty($globals['disable_current_ins'])){
		...
		foreach($installations[$soft] as $ik => $iv){
			...
			$info['overview'] .= '<tr id="trid' . $ik . '">
				...
				<td><a href="'.$iv['softurl'].'/'.(!empty($iv['admin_folder']) ? $iv['admin_folder'] : admin_folder($iv['sid'])).'" target="_blank" title="'.$l['login_exp'].'" class="someclass"><i class="fas fa-user px-1 sai_icons"></i></a></td>
				<td>'.datify($iv['itime']).'</td>
				<td>'.$iv['ver'].(!empty($is_upgradable) ? ' <a href="'.$globals['ind'].'act=perl_upgrade&insid='.$ik.'" title="'.$l['upd_to'].' '.implode(' or ', $is_upgradable).'" class="someclass"><i class="fas fa-sync-alt px-1 sai_icons" style="color:#1565C0; font-size:1.2em;"></i></a>' : '').'</td><td align="right">'.
				...
				(empty($globals['disable_backup_restore']) ? '<a href="'.$globals['ind'].'act=backup&insid='.$ik.'" title="'.$l['backup'].'" class="someclass mr-1"><i class="fas fa-file-archive px-1 sai_icons" style="color:#119768;"></i></a>' : '').' &nbsp;<a href="'.$globals['ind'].'act=editdetail&insid='.$ik.'" title="'.$l['editdetail'].'" class="someclass"><i class="fas fa-pencil-alt px-1 sai_icons" style="color:#FF5D00;"></i></a>'.(asperapp(0, 0, 1) && !empty($can_publish) ? ' &nbsp;<a href="'.$globals['ind'].'act=publish&insid='.$ik.'" title="'.$l['publish'].'" class="someclass"><i class="fas fa-cloud-upload-alt px-1 sai_icons" style="color:#00A0D2;"></i></a>' : '').' &nbsp;<a href="'.$globals['ind'].'act=remove&insid='.$ik.'" title="'.$l['remove'].'" class="someclass"><i class="fas fa-trash-alt sai_icons" style="color:red;"></i></a>
				<td>
