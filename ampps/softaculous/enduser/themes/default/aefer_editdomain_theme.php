...
function editdomain_theme(){

global $user, $globals, $l, $theme, $softpanel, $iscripts, $catwise, $error;
global $saved, $thisdomain;

softheader($l['<title>']);

echo '<div class="bg"><br />';

echo '<form accept-charset="'.$globals['charset'].'" name="editadomain" method="post" action="" class="form-horizontal">
	<div class="sai_main_head" style="width:100%;" align="center">'.$l['edit_domain'].'</div>';
	...
	<div class="row">
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			...
			<input type="text" name="domain" id="domain" class="form-control" value="'.POSTval('domain', $thisdomain['domain']).'" />
		</div>
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			...
			<input type="text" name="server_host" id="server_host" class="form-control" value="'.POSTval('server_host', $thisdomain['server_host']).'" />
		</div>
	</div><br />
	...
	<div class="row">
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			...
			<select name="protocol" class="form-control" id="protocol">
				<option value="ftp" '.(isset($_POST['protocol']) && $_POST['protocol'] == 'ftp' ? 'selected="selected"' : ($thisdomain['protocol'] == 'ftp' ? 'selected="selected"' : '')).'>FTP</option>
				<option value="ftps" '.(isset($_POST['protocol']) && $_POST['protocol'] == 'ftps' ? 'selected="selected"' : ($thisdomain['protocol'] == 'ftps' ? 'selected="selected"' : '')).'>FTPS</option>
				<option value="sftp" '.(isset($_POST['protocol']) && $_POST['protocol'] == 'sftp' ? 'selected="selected"' : ($thisdomain['protocol'] == 'sftp' ? 'selected="selected"' : '')).'>SFTP</option>
			</select>
		</div>
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			<label for="port" class="sai_head">'.$l['port'].'</label><br />
			<span class="sai_exp2">'.$l['port_exp'].'</span>
			<input type="text" name="port" id="port" class="form-control" value="'.POSTval('port', $thisdomain['port']).'" />
		</div>
	</div><br />
	...
	<div class="row" >
		<div class="col-12 col-md-6 col-xl-6 mb-2" id="trauthpass">
			...
			&nbsp; &nbsp; 
			<input type="radio" name="auth_password" id="auth_password" value="1" onchange="check_auth_password();" '.POSTradio('auth_password', 1, (!empty($thisdomain['ftp_pass']) ? 1 : null)).' />&nbsp;'.$l['auth_method_pass'].' &nbsp;&nbsp;
			<input type="radio" name="auth_password" id="auth_password" value="" onchange="check_auth_password();" '.POSTradio('auth_password', '', (!empty($thisdomain['private_key']) ? '' : null)).' />&nbsp;'.$l['auth_method_key'].'<br />
		</div><br />
	</div><br/>
	...
	<div class="row">
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			...
			<input type="text" name="ftp_user" id="ftp_user" class="form-control" value="'.POSTval('ftp_user', $thisdomain['ftp_user']).'" />
		</div>
		<div class="col-12 col-md-6 col-xl-6 mb-2" id="ftp_pass">
			...
			<span class="sai_exp2">'.$l['ftp_pass_exp'].((empty($globals['no_prefill_pass'])) ? '' : '<br/>'.$l['ftp_pass_opt']).'</span>
			<div class="row">
				<div class="col-sm-11">
					<input type="password" name="ftp_pass" id="ftp_passwd" class="form-control" value="'.POSTval('ftp_pass', (empty($globals['no_prefill_pass']) ? $thisdomain['ftp_pass'] : '')).'" autocomplete="off" />
				</div>
	...
	.((empty($globals['no_prefill_pass'])) ? '' : '<br/>').
	'<div class="row">
		<div class="col-12 col-md-6 col-xl-6 mb-2" id="pri">
			...
			<textarea name="private_key" id="private_key" rows="15" cols="70" class="form-control">'.POSTval('private_key', $thisdomain['private_key']).'</textarea><br />
		</div>
		<div class="col-12 col-md-6 col-xl-6 mb-2" id="passphrase_div">
			...
			<input type="text" name="passphrase" id="passphrase" class="form-control" value="'.POSTval('passphrase', $thisdomain['passphrase']).'" /><br />
		</div>
	</div>
	...
	<div class="row">
		<div class="col-12 col-md-6 col-xl-6 mb-2">
			...
			<input type="text" name="ftp_path" id="ftp_path" class="form-control" value="'.POSTval('ftp_path', $thisdomain['ftp_path']).'" />
		</div>';
	...
	if(empty($softpanel->dont_show_backup_dir) && empty($globals['disable_backup_restore'])){
		...
		echo '<div class="col-12 col-md-6 col-xl-6 mb-2">
				...
				<input type="text" name="backup_path" id="backup_path" class="form-control" value="'.POSTval('backup_path', $thisdomain['backup_dir']).'" />
			</div>';
	}
	...
	if(empty($softpanel->dont_show_datadir)){
		echo '<div class="row">
			<div class="col-12 col-md-6 col-xl-6 mb-2">
				...
				<input type="text" name="data_dir" id="data_dir" class="form-control" value="'.POSTval('data_dir', $thisdomain['data_dir']).'" />
			</div>
		</div><br />';
	}	
	...
	echo '
		<p align="center">
			<br /><br />
			'.csrf_display().'
			<input class="flat-butt" type="submit" name="edit_domain" value="'.$l['submit_edit'].'" />
			<br /><br />
