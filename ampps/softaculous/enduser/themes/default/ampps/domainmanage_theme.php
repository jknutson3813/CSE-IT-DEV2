...
<div class="row sai_main_head" style="width:100%;" align="center">
	<div class="col-sm-5 col-xs-5" style="padding:0 10px 0 0; text-align:right;">
		<img height="48" width="48" src="'.$theme['a_images'].'domains.png" />
	</div>
	<div class="col-sm-7 col-xs-7" style="padding-top:10px; padding-left:0; text-align:left;">'.$l['domain_headling'].'</div>
</div>
<hr>';
	echo '
		
		<div class="bg-2" id="domainlist">
			';domainlist(); echo ' 
		</div>
		...
		<div class="row my-5">
			<div class="col-12 px-4 text-center">
				<p class="btn sai-button"><a href="'.$globals['ind'].'act=ampps_domainadd" style="text-decoration:none; color:#fff;">'.$l['add_new'].'</a></p>
			</div>
	</div>
...
function hosttable(){
	...
	echo '<table border="0" cellpadding="0" cellspacing="0" width="100%" class="table table-hover">
	<tr>
...
function domainlist(){
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="table table-hover">
		...
		foreach ($domains_list as $key => $value){
			...
			if ($key != $primary_domain)
			{
				echo '<td><i class="delete fas fa-trash-alt px-2 sai_icons" title="Delete" style="cursor:pointer;color:red;" id="did'.$i.'"></i>';
			}else{
				echo '<td>-</td>';
			}
			$i++;
