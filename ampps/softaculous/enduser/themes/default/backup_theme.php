...
$return .= '<a '.$role.' href="'.script_link($soft).'&highlight='.$insid.'&postact=backup"><span class="btn btn-outline-primary">'.$l['return'].'</span></a>';
...
echo '<div class="bg backup-form"><br />
			...
			echo '<div class="row">
				<div class="col-12 my-2 text-center">
					<div class="alert alert-warning mt-4">
						...
						<span>'.lang_vars($l['backups_expire'], array($globals['backups_expire'])).'</span>
					</div>
				</div>
			</div>';
		}
		...
		if(!empty($globals['max_backups'])){
			...
			<div class="col-12 my-2 text-center">
				<div class="alert alert-warning">
					<a href="#close" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					<span>'.lang_vars($l['max_backups'], array($cur_backups, $globals['max_backups'])).'</span>
				</div>
			</div>
		</div>';
		}
		...
		if(!empty($globals['max_backup_script']) && $backup_count_per_script >= $globals['max_backup_script']){
			...
			<div class="col-12 my-2 text-center">
				<div class="alert alert-warning">
					<a href="#close" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					<span>'.lang_vars($l['error_max_backup_script'], array($globals['max_backup_script'])).'</span>
				</div>
			</div>
		</div>';	
		} 
		...
		<div class="mt-4">
			<div class="m-3 card p-3">
				<div class="sai_sub_head">'.$l['backup_operation'].'</div>
				<hr class="sai_main_head_hr">
					<div class="row mb-2">
						<div class="col-sm-5">
							<label class="sai_head" for="backup_dir">'.$l['backup_dir'].'</label><br />
							<span class="sai_exp2">'.$l['backup_dir_exp'].'</span>
						</div>
						<div class="col-sm-2">
							<input type="checkbox" name="backup_dir" id="backup_dir" '.POSTchecked('backup_dir', true).' />
						</div>
					</div>';
				...
				if(!empty($user['ins'][$insid]['wwwdir'])){
					...
					<div class="row mb-2">
						<div class="col-sm-5">
							<label class="sai_head" for="backup_wwwdir">'.$l['backup_wwwdir'].'</label><br />
							<span class="sai_exp2">'.$l['backup_wwwdir_exp'].'</span>
						</div>
						<div class="col-sm-2">
							<input type="checkbox" name="backup_wwwdir" id="backup_wwwdir" '.POSTchecked('backup_wwwdir', true).' />
						</div>
					</div>';
				}
				...
				if(!empty($user['ins'][$insid]['softdatadir'])){
					...
					<div class="row mb-2">
						<div class="col-sm-5">
							<label class="sai_head" for="backup_datadir">'.$l['backup_datadir'].'</label><br />
							<span class="sai_exp2">'.$l['backup_datadir_exp'].'</span>
						</div>
						<div class="col-sm-2">
							<input type="checkbox" name="backup_datadir" id="backup_datadir" '.POSTchecked('backup_datadir', true).' />
						</div>
					</div>';
				}
				...
				if(!empty($user['ins'][$insid]['softdb'])){
					...
					<div class="row mb-2">
						<div class="col-sm-5">
							<label class="sai_head" for="backup_db">'.$l['backup_db'].'</label><br />
							<span class="sai_exp2">'.$l['backup_db_exp'].'</span>
						</div>
						<div class="col-sm-2">
							<input type="checkbox" name="backup_db" id="backup_db" '.POSTchecked('backup_db', true).' />
						</div>
					</div>';
				}
				...
				echo '
					<div class="row mb-2">
						<div class="col-sm-5">
							<label for="backup_note" class="sai_head mb-2">'.$l['backup_note'].'</label>
							<span class="sai_exp">'.$l['backup_note_exp'].'</span>
						</div>
						<div class="col-sm-7">
							<input type="text" name="backup_note" id="backup_note" class="form-control" '.POSTval('backup_note', '').' size="50" maxlength="255"/><br />
						</div>
					</div>';
		...
		<div class="row" align="center">
			<div class="col-12 text-center mt-4">
				...
				<input type="submit" name="softsubmitbut" class="btn sai-button" id="softsubmitbut" value="'.$l['backup_ins'].'"  />
				<br /><br /><center><p class="m-3">'.$return.'</p></center>
			</div>
		</div>
	...
	<div class="bg">
		...
		<div class="row sai_main_head" style="width:100%;" align="center">
			<div class="col-sm-5 col-xs-5" style="padding:0 10px 0 0; text-align:right;">
				<i class="far fa-file-archive fa-2x" style="color:#00A0D2;"></i>
			</div>
