...
function staging_theme(){
	...
	if(can_wordpress_manager($soft)){
		$return .= '<a id="staging_wp_management" role="button" href="'.$globals['ind'].'act=wordpress&insid='.$new_insid.'"><span class="btn btn-outline-primary">'.$l['return_to_wpm'].'</span></a>&nbsp';
		
		$role = 'role="button"';
	}

	$return .= '<a id="staging_overview" '.$role.' href="'.script_link($soft).'&highlight='.$new_insid.'&postact=staging"><span class="btn btn-outline-primary">'.$l['return'].'</span></a>';
	...
	echo '
	<div class="bg" id="completed" '.(empty($staged) ? 'style="display:none;"' : '').'>
		<h5>'.$l['congrats'].'</h5>
		...
		<div class="setupcontinue">'.(!empty($setupcontinue) || empty($staged) ? $l['setup_continue'].' : <br />
		...
		if(!empty($error)){
			echo '<div id="error_handle"></div>';
		}
	
		if(empty($globals['staging_tweet_off']) || empty($globals['lictype'])){
			echo '
			<form method="get" action="https://twitter.com/intent/tweet" id="tweet" onsubmit="return dotweet(this);">
				...
				<div class="card">
					<div class="card-header" style="font-size:14px;">
						<font><b>'.$l['staging_tweet_sub'].' :</b></font>
					</div>
					<div class="card-body">
						<div class="row">
							<div class="col-sm-12">
								<textarea id="txttweet" name="text" cols="80" row="3" style="resize:none; width:100%;">'.loadtweetdata('staging_tweet', $l['staging_tweet']).'</textarea>
							</div>
						</div>
						<br />
						<div class="row">
				...
				<form accept-charset="'.$globals['charset'].'" name="clonesoftware" method="post" action="" onsubmit="return checkform();" id="clonesoftware">
							...
							$.ajax({
								...
								success: function(data){
									...
									if(data.substring(0,5) == "error"){
										...
										//Handling when error occured before pushing in background(progress div was also visible)
									...
									if(tmp[2] != null){
										...
										var staging_overview = background_variables.staging_overview;
										var staging_wp_management = background_variables.staging_wp_management;
									}
									...
									if(tmp[0] == 100 && progressbar.backgrounding != null && progressbar.backgrounding != undefined){
										...
										$("#staging_overview").attr("href", staging_overview);
										if($("#staging_wp_management") !== null){
											$("#staging_wp_management").attr("href", staging_wp_management);
										}
								...
								complete: function( jqXHR, status, responseText ) {
									...
									if(backgrounding.length > 0){
										...
										if ( jqXHR.state() == "resolved" ) {
											...
											var err = $(responseText).find(\'#error_handle\');
					...
					<div class="bg"><br />						
						<div class="row sai_main_head mt-8 mb-4">
							<div class="col-12 text-center">
								<img src="'.$theme['images'].'staging_48.png" class="mr-2" /><span>
							'.$l['prog_staging'].$software['name'].'</span>
							</div>
						</div>';
						...
						//Maximum Staging Limit per script
						if(!empty($globals['max_staging_script']) && $staging_count_per_script >= $globals['max_staging_script']){
							echo '
							<div class="alert alert-warning mt-4">
								<a href="#close" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<center>'.lang_vars($l['error_max_staging_script'], array($globals['max_staging_script'])).'</center>
							</div>';	
						}

						if(!can_use_soft($soft, $use_msg)){
							echo '<center class="alert alert-warning mt-4">'.$use_msg.'</center>';
						}
						
						//Set Maximum Staging Limit for user
						if(!empty($globals['max_staging']) && $GLOBALS['staging_count'] >= $globals['max_staging']){
							echo '
							<div class="alert alert-warning mt-4">
								<a href="#close" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<center>'.lang_vars($l['max_staging_error'], array($globals['max_staging'])).'</center>
							</div>';	
						} 
							
						echo error_handle($error, "100%", 0, 1);
					
						// The Source installation details form
						echo '
						<div class="mt-4">
						<div class="card p-3 m-3"><br />
						
						<div class="sai_sub_head">'.$l['staging_ins'].'</div>
						<hr class="sai_main_head_hr" />
						
						<div class="row mb-2">
							<div class="col-sm-5">
								<label for="softdomain" class="sai_head">'.$l['live_ins_url'].'</label><br />
									<span class="sai_exp2">'.$l['live_ins_url_exp'].'</span>
							</div>
							<div class="col-sm-7">
								<span class="sai_info"><a href="'.$user['ins'][$insid]['softurl'].'" target="_blank" style="text-decoration:none;">'.$user['ins'][$insid]['softurl'].'</a></span>
							</div>
						</div><br />
						
						<div class="row mb-2">
							<div class="col-sm-5">
								<label for="softdirectory" class="sai_head">'.$l['choose_url'].'</label><br />
							...
							if(empty($softpanel->noprotocol)){
								...
								<div class="col-sm-3 custom_install" style="padding-left:0px; padding-right:1px; width:24%;">
									...
									echo '</select>
									<span class="sai_exp2" style="margin-left:4px;">'.$l['choose_protocol'].'
							...
							<div class="col-sm-6" style="padding-left:0px; padding-right:0px;">
								...
								<span class="sai_exp2" style="margin-left:4px;">'.$l['choose_domain'].'
							...
							<div class="row col-sm-12">
								...
								<span id="httpserror" style="display:none;" class="alert alert-warning my-2"></span>
							</div>
							</div>
							</div>
						</div><br />';
						...
						if(!empty($cron)){
							...
							<div class="row mb-2">
								<div class="col-sm-5">
									<label for="cron_min" class="sai_head">'.$l['cron_job'].'</label><br />
						...
						if(!empty($settings)){
							...
							<div class="card p-3 m-3">';
								...
								foreach($settings as $group => $sets){
									...
									echo '
									<div class="sai_sub_head">'.$group.'</div><hr class="sai_main_head_hr">';
									...
									foreach($sets as $sk => $sv){
										...
										<div class="row mb-2">
											<div class="col-sm-5">
												...
												'.(empty($sv['exp']) ? '' : '<span class="'.$exp_class.'">'.$sv['exp'].'</span>').'
											</div>';
					...
					<div class="row mt-4">
						<div class="col-12 text-center">
							...
							<input type="hidden" name="stagingins" id="stagingins" value="'.$l['create_staging'].'" />
							<input type="submit" name="softsubmitbut" class="btn sai-button" id="softsubmitbut" value="'.$l['create_staging'].'" />
					...
					<center>
						<div class="row sai_main_head" style="width:100%;" align="center">
							<div class="col-sm-5 col-xs-5" style="padding:0 10px 0 0; text-align:right;">
								<img src="'.$theme['images'].'staging_48.png" />
							</div>
							<div class="col-sm-7 col-xs-7" style="padding-top:10px; padding-left:0; text-align:left;">'.$l['prog_staging'].$software['name'].'</div>
						</div><hr /><br />
