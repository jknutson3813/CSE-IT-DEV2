...
function sclone_theme(){
	...
	softheader($l['<title>']);
		...
		if(can_wordpress_manager($soft)){
			$return .= '<a id="sclone_wp_management" role="button" href="'.$globals['ind'].'act=wordpress&insid='.$new_insid.'"><span class="btn btn-outline-primary">'.$l['return_to_wpm'].'</span></a>&nbsp';
			
			$role = 'role="button"';
		}

		$return .= '<a id="sclone_overview" '.$role.' href="'.script_link($soft).'&highlight='.$new_insid.'&postact=clone"><span class="btn btn-outline-primary">'.$l['return'].'</span></a>';
							...
							$.ajax({
								...
								success: function(data){
									...
									if(tmp[2] != null){
										...
										var sclone_overview = background_variables.sclone_overview;
										var sclone_wp_management = background_variables.sclone_wp_management;
									}
									...
									if(tmp[0] == 100 && progressbar.backgrounding != null && progressbar.backgrounding != undefined){
										...
										$("#sclone_overview").attr("href", sclone_overview);
										if($("#sclone_wp_management") !== null){
											$("#sclone_wp_management").attr("href", sclone_wp_management);
										}
										progressbar.sync_abort();	
					...
					<div class="bg"><br />						
						<div class="row sai_main_head mt-8 mb-4">
							<div class="col-12 text-center">
								<i class="fas fa-copy fa-2x mr-2" style="color:#00A0D2;"></i>
								<span>'.$l['prog_cloning'].$software['name'].'</span>
							</div>
						</div>';
						...
						<div class="card p-3 m-3"><br />
							<div class="sai_sub_head">'.$l['clone_ins'].'</div>
							<hr class="sai_main_head_hr" />
						
							<div class="row mb-2">
								<div class="col-sm-5">
									<label class="sai_head" for="softdirectory">'.$l['choose_url'].'</label><br />
								...
								<div class="row col-sm-12">
									<div style="display:none;" id="checkhttps_wait">
										<img src="'.$theme['images'].'progress.gif" alt="Please wait.." class="ml-2">
									</div>
						...
						<div class="row mt-4">
							<div class="col-12 text-center">
								...
								<input type="submit" name="softsubmitbut" class="flat-butt btn sai-button" id="softsubmitbut" value="'.$l['cloneins'].'" />
