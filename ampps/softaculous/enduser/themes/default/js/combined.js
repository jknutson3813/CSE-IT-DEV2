...
function new_theme_funcs_init(){
	...
	$(".sai_exp").each(function(){
		...
		var str = $(this).parent().find(".fa-info-circle").length;
		// If we do not find the sai-info we will add it twice(as this function is called twice we do not want to add double info class)
		if(!str){
			var str1 = "&nbsp;<a data-toggle=\"tooltip\" data-html=\"true\" title=\""+txt+"\"><i class=\"fas fa-info-circle\" style=\"font-size:1.1em; vertical-align:middle;cursor:pointer;color:#4B4B4B;\"></i></a>";
			//need to add sai-info class in <i>
			$(this).before(str1);
		}
	});
	 
	$(".fa-info-circle").mouseover(function() {
		$(this).parent().find(".sai_exp").fadeIn(5);
	});
	
	$( ".fa-info-circle,.sai_exp" ).mouseout(function() {
		$(this).parent().find(".sai_exp").fadeOut();	
			$(".sai_exp").mouseover(function(){
				$(".sai_exp").stop();
			});
	});
	
	//For Tooltips (only available in Bootstrap 4)
	try{
		$('[data-toggle="tooltip"]').tooltip({trigger:"hover"});
		$('[data-toggle="tooltip"]').click(function(){$('[data-toggle="tooltip"]').tooltip("hide")});
	}catch(e){
		// Do nothing. admin panel will be updated to Bootstrap 4 soon
	}
...
function shift_check_all(check_class){ 
...
/*
 * bootstrap-growl
 * Turn Bootstrap alerts into "Growl-like" notifications
