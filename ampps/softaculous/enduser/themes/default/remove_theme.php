	...
	sync: function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				//Hide the return buttons while under process
				if($("#remove_return_buttons") !== null){
					$("#remove_return_buttons").hide();
				}
			}
		});
	},
...
<div class="row sai_main_head mt-8 mb-4" style="width:100%;" align="center">
	<div class="col-12 text-center">
		<i class="fas fa-times fa-2x mr-2" style="color:#00A0D2;"></i>
		<span class="form-title">'.$l['prog_removing'].$software['name'].'</span>
	</div>
</div>';
...
if(empty($globals['remove_dir'])){
	...
	echo '<div class="row mb-3">
		...
		<div class="col-sm-2">
			<input type="checkbox" name="remove_dir" id="remove_dir" '.POSTchecked('remove_dir', (empty($info_only) ? true : false)).' />
		</div>
	</div><br />';
...
if(!empty($user_ins['softdatadir']) && empty($globals['remove_datadir'])){
	...
	echo '<div class="row mb-3">
		...
		<div class="col-sm-2">
			<input type="checkbox" name="remove_datadir" id="remove_datadir" '.POSTchecked('remove_datadir', (empty($info_only) ? true : false)).' />
		</div>
	</div><br />';
...
if(!empty($user_ins['wwwdir']) && empty($globals['remove_wwwdir'])){
	...
	echo '<div class="row mb-3">
		...
		<div class="col-sm-2">
			<input type="checkbox" name="remove_wwwdir" id="remove_wwwdir" '.POSTchecked('remove_wwwdir', (empty($info_only) ? true : false)).' />
		</div>
	</div><br />';
...
if(!empty($user_ins['softdb']) && empty($globals['remove_db'])){
	...
	echo '<div class="row mb-3">
		...
		<div class="col-sm-2">
			<input type="checkbox" name="remove_db" id="remove_db" '.POSTchecked('remove_db', (empty($info_only) ? true : false)).' />
		</div>
	</div><br />';
...
if(!empty($user_ins['softdbuser']) && empty($globals['remove_db'])){
	...
	echo '<div class="row mb-3">
		...
		<div class="col-sm-2">
			<input type="checkbox" name="remove_dbuser" id="remove_dbuser" '.POSTchecked('remove_dbuser', (empty($info_only) ? true : false)).' />
		</div>
	</div><br />';
...
<div class="bg m-3 info_block">
	<div class="sai_sub_head">'.$l['info'].'</div><hr class="sai_main_head_hr" style="width:50px;">
	...
	'.(empty($user_ins['ver']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['softurl']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['softdatadir']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['wwwdir']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['softdb']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['softdbuser']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['softdbhost']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
	...
	'.(empty($user_ins['cron_command']) ? '' : '<div class="row mb-3">
		<div class="col-sm-5">
...
<input type="submit" name="softsubmitbut" class="btn sai-button" id="softsubmitbut" value="'.$l['removeins'].'"  />
</p>
	...
	<center>
		<div class="row sai_main_head" style="width:100%;" align="center">
			<div class="col-sm-5 col-xs-5" style="padding:0 10px 0 0; text-align:right;">
				<i class="fas fa-times fa-2x" style="color:#00A0D2;"></i>
			</div>
...
if(!empty($insid)){
	...
	if(can_wordpress_manager($soft)){
		$return .= '<a role="button" href="'.$globals['ind'].'act=wordpress'.(!empty($removed) ? '' : '&insid='.$insid).'"><span class="btn btn-outline-primary">'.$l['return_to_wpm'].'</span></a>&nbsp';
	...
	echo '<br /><div id="remove_return_buttons"><center>'.$return.'</center><br /><br /></div>';
