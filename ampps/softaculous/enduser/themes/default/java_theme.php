...
if((empty($globals['default_protocol']) || $globals['default_protocol'] <= 1) && empty($globals['off_checkhttps'])){
	...
	function check_https(){
		$.ajax({
			type: "POST",
			url: "'.$globals['index'].'act=java&soft='.$soft.'&checkhttps="+encodeURIComponent($("#softdomain").val()),
			timeout:10000,
	...
	if(nopackage){
		...
		try{
			$.ajax({
				...
				url: "'.$globals['index'].'act=java&soft='.$soft.'&ajaxdownload=1&random="+Math.random(),
				// Checking for error
				success: function(data){
...
function checkhttps(proto_id, softdomain_id, alrt){
	try{
		...
		if(proto.indexOf("https") !== -1){
			 ...
			 $.ajax({
				type: "POST",
				url: "'.$globals['index'].'act=java&soft='.$soft.'&checkhttps="+encodeURIComponent(no_proto+$_(softdomain_id).value),
				timeout:10000,
				// Checking for error
				success: function(data){
...
if(!empty($installations[$soft])){
	...
	foreach($installations[$soft] as $ik => $iv){
		...
		$info['overview'] .= '<tr id="trid' . $ik . '">
					...
					<td>
						<a href="'.$iv['softurl'].'/'.(!empty($iv['admin_folder']) ? $iv['admin_folder'] : admin_folder($iv['sid'])).'" target="_blank" title="'.$l['login_exp'].'" class="someclass"><i class="fas fa-user px-1 sai_icons"></i></a>
					</td>
					<td>
						'.datify($iv['itime']).'
					</td>
