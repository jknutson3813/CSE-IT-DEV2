...
function restore_theme(){
	...
	global $user, $globals, $l, $theme, $softpanel, $iscripts, $catwise, $error, $scripts, $_insid, $dbexist, $restored, $datadir, $wwwdir, $backupdir, $restoring;
	...
	if(can_wordpress_manager($soft)){
		$return .= '<a role="button" href="'.$globals['ind'].'act=wordpress&insid='.$_insid[1].'"><span class="btn btn-outline-primary">'.$l['return_to_wpm'].'</span></a>&nbsp';
		
		$role = 'role="button"';
	}

	$return .= '<a '.$role.' href="'.script_link($soft).'&highlight='.$_insid[1].'&postact=restore"><span class="btn btn-outline-primary">'.$l['return'].'</span></a>';
				...
				<div class="bg"><br />
					...
					<div class="row sai_main_head mt-8 mb-4" style="width:100%;" align="center">
						<div class="col-12 text-center">
							<i class="fas fa-undo fa-2x mr-2" style="color:#00A0D2;"></i>
							<span class="form-title">'.$l['prog_restoring'].$iscripts[$soft]['name'].'</span>
						</div>
					</div>';
					...
					echo '<div class="m-3 card p-3">';
					
					if(!empty($backupdir)){
						echo '
						<div class="row mb-3">
							<div class="col-sm-7">
								<label class="sai_head" for="restore_dir">'.$l['restore_dir'].'</label><br />
								<span class="sai_exp2">'.$l['restore_dir_exp'].'</span>
							</div>
							<div class="col-sm-2">
								<input type="checkbox" name="restore_dir" id="restore_dir" '.POSTchecked('restore_dir', true).' value="1" />
							</div>
						</div><br />
						
						<div class="row mb-3">
							<div class="col-sm-7">
								<label class="sai_head" for="restore_delete_files">'.$l['restore_delete_files'].'</label><br />
								<span class="sai_exp2">'.$l['restore_delete_files_exp'].'</span>
							</div>
							<div class="col-sm-2">
								<input type="checkbox" name="restore_delete_files" id="restore_delete_files" '.POSTchecked('restore_delete_files', false).' value="1" />
							</div>
						</div><br />';
					...
					if(!empty($wwwdir)){
						...
						<div class="row mb-3">
							<div class="col-sm-7">
								<label class="sai_head" for="restore_wwwdir">'.$l['restore_wwwdir'].'</label><br />
								<span class="sai_exp2">'.$l['restore_wwwdir_exp'].'</span>
							</div>
							<div class="col-sm-2">
								<input type="checkbox" name="restore_wwwdir" id="restore_wwwdir" '.POSTchecked('restore_wwwdir', true).' value="1" />
							</div>
						</div><br />';		
					}
					...
					if(!empty($datadir)){
						...
						<div class="row mb-3">
							<div class="col-sm-7">
								<label class="sai_head" for="restore_datadir">'.$l['restore_datadir'].'</label><br />
								<span class="sai_exp2">'.$l['restore_datadir_exp'].'</span>
							</div>
							<div class="col-sm-2">
								<input type="checkbox" name="restore_datadir" id="restore_datadir" '.POSTchecked('restore_datadir', true).' value="1" />
							</div>
						</div><br />';	
					}
					...
					if(!empty($dbexist)){
						...
						<div class="row mb-3">
							<div class="col-sm-7">
								<label class="sai_head" for="restore_db">'.$l['restore_db'].'</label><br />
								<span class="sai_exp2">'.$l['restore_db_exp'].'</span>
							</div>
							<div class="col-sm-2">
								<input type="checkbox" name="restore_db" id="restore_db" '.POSTchecked('restore_db', true).' value="1" />
							</div>
						</div><br />';		
					}
					...
					<p align="center">
						...
						<input type="submit" name="restorebtn" class="btn sai-button" id="restorebtn" value="'.$l['restore_ins'].'"  />
					</p>
					<br />
					<center>'.$return.'</center><br /><br />				
				...
				<center>
					<div class="row sai_main_head" style="width:100%;" align="center">
						<div class="col-sm-5 col-xs-5" style="padding:0 10px 0 0; text-align:right;">
							<i class="fas fa-undo fa-2x" style="color:#00A0D2;"></i>
						</div>
