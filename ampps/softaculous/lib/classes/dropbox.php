...
class dropbox{
	...
	function upload_append($session_id, $filep, $data_size){
		...
		$current_time = time() - $this->refresh_time;
		
		// Refreshing token after every 3.5 hours, since dropbox tokens are short-lived for 4hrs
		if($current_time >= 12600){
			$this->refresh_time = time();
			$this->access_token = $this->refresh_token_func($this->refresh_token);
		}
	...
	function generate_dropbox_token($auth_code){
		...
		$drop_data['access_token'] = $resp['access_token'];
		$drop_data['refresh_token'] = $resp['refresh_token'];
		
		return $drop_data;
	}
	
	//New (Since September 30th, 2021) Dropbox access token expires in 4 hours so we need to refresh
	function refresh_token_func($refresh_token){
		global $globals, $dropbox, $l, $error;
		...
		if(empty($resp)){
			$error[] = $l['dropbox_refresh_token_err'];
			return false;
		}
	...
	function __curl($url, $headers = '', $filepointer = '', $upload_size = 0, $post = '', $download_file = '', $ignore_errors = 0, $retry = false){
		...
		if(empty($resp) && empty($retry) && empty($http_code)){
			$res = $this->__curl($url, $headers, $filepointer, $upload_size, $post, $download_file, $ignore_errors, true);
		}
		...
		if(!empty($result['error']) && empty($ignore_errors)){
			if($result['error']['.tag'] == 'invalid_access_token'){
				...
				$error[$result['error']['.tag']] = $l['incorrect_offset'];
