...
global $__settings, $error, $software, $globals, $notes, $setupcontinue;
	
	//Need this file to unlock the installer. We need it to run migrations on database 
	swrite($__settings['softpath'].'/.NO_SECURE_KEY', '', 1);
	
	@schmod($__settings['softpath'].'/.NO_SECURE_KEY', $globals['ofc']);
	@schmod($__settings['softpath'].'/installed.log', $globals['ofc']);
	
	//Permission needed to generate new APP_KEY
	@schmod($__settings['softpath'].'/.env', $globals['ofc']);
	
	$notes = 'Please visit the above url to upgrade the database. Also apply safe permissions to the environment file<br /><b>'.$__settings['softpath'].'/.env</b> and also please delete file <b>'.$__settings['softpath'].'/.NO_SECURE_KEY</b> after the upgrade is complete.';
}
