...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `usr_id` FROM `".$__settings['dbprefix']."_users` WHERE `usr_login_name` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."_users` SET `usr_password` = '".$__settings['admin_pass']."' WHERE `usr_id` = '".$userid."';";
